...
import {paramType} from "@/api/modules";
...
export const statisticsProvinceNum = async (params) => {
    try {
        // 加密参数
        params = isEncryptionParam(params)
        const data = await axios.get(`http://localhost:9090/piece/statisticsProvinceNum`, {
            params: params,
            headers: configs_ENC.headers
        }, configs_ENC);
        return data;
...
export const statisticsDiseaseTypesCounts = async (params) => {
    try {
        // 加密参数
        params = isEncryptionParam(params)
...
export const getGeoInformationByIP = async (params) => {
    try {
        // 加密参数
        params = isEncryptionParam(params)
        const data = await axios.get(`/piece/lnglat`, {
            params: params,
            headers: configs_ENC.headers
        }, configs_ENC);
        return data;
